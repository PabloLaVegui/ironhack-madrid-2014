...
RSpec.describe "Places Popularity Ranking (RankingGenerator class)" do
  ...
  it "Ranking by number of visits" do
    expect(RankingGenerator.new(ranking_num_of_visits).sorted_elements).to eq([place_3, place_2, place_1])
  end

  it "Ranking by number of comments" do
    expect(RankingGenerator.new(ranking_num_of_comments).sorted_elements).to eq([place_3, place_2, place_1])
  end

  it "Ranking by number of a letters in name" do
    expect(RankingGenerator.new(ranking_num_of_a_letter).sorted_elements).to eq([place_2, place_1, place_3])
  end

  it "Ranking by number of words comments" do
    expect(RankingGenerator.new(ranking_num_of_words_in_comments).sorted_elements).to eq([place_3, place_2, place_1])
  end
